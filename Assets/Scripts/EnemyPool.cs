...
public class EnemyPool : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for(int i=0;i<poolSize;i++){
            enemyPool[i] = (GameObject)Instantiate(enemy,enemyPoolPos,Quaternion.identity);
        }
    }
    ...
    void Update()
    {
        ...
        if(!GameControl.Instance.isGameOver && timeSinceLastSpawn >= spawnRates[currentSpawn]){
            ...
            enemyPool[currentEnemy].transform.position = new Vector2(spawnXPos + r,spawnYPos);
            //make trigger and visibility enabled again.
            enemyPool[currentEnemy].GetComponent<Collider2D>().enabled = true;
            enemyPool[currentEnemy].GetComponent<Renderer>().enabled = true;
            enemyPool[currentEnemy].GetComponent<Enemy>().isDead = false;
            currentEnemy++;
